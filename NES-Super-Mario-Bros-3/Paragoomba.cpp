...
void CParagoomba::ChangeAnimation()
{
	...
	int action = ANI_ID_PARAGOOMBA_WALK;
	if (state == STATE_DIE)
	{
		action = ANI_ID_PARAGOOMBA_DIE;
	}
...
void CParagoomba::ChaseMario()
{
	if (momentum < 1)
	{
		if (isOnGround && GetTickCount64() - start_momentum >= PARAGOOMBA_CHASE_RUN_DURATION)
		{
