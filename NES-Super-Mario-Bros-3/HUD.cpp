...
CHud::CHud()
{
	...
	for (int i = 0; i < MOMENTUM_COUNT; i++)
	{
		...
		momentumBar[i] = new CPMeter(pType, false);
	}
	for (int i = 0; i < HUD_CARD_COUNT; i++)
	{
		cards[i] = new CHUDCard(CARD_EMPTY);
	}
}
...
void CHud::UpdateMomentum(int momentum)
{
	for (int i = 0; i < MOMENTUM_COUNT; i++)
		momentumBar[i]->SetToggle(false);
	for (int i = 0; i < momentum; i++)
		momentumBar[i]->SetToggle(true);
}

void CHud::UpdateCards(int cardType[])
{
	for (int i = 0; i < HUD_CARD_COUNT; i++)
		cards[i]->SetType(cardType[i]);
}
...
void CHud::SetPosition(float x, float y)
{
	...
	for (int i = 0; i < MOMENTUM_COUNT; i++)
	{
		...
		momentumBar[i]->SetPosition((int)(x + pOffset + MOMENTUM_OFFSET + i * DIGIT_NEAR_SPACING), (int)(y + OFFSET_Y_LINE1));
	}
	for (int i = 0; i < HUD_CARD_COUNT; i++)
	{
		cards[i]->SetPosition(x + CARD_OFFSET + i * CARD_NEAR_SPACING, y + OFFSET_Y_LINE3);
	}
}
